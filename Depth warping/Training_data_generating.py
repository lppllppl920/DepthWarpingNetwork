...
#        translation_vector = np.zeros((3, 3), dtype = 'float32')
    ...
    for n in range(0, len(poses) - 1):
        num = np.min([5, len(poses) - n])
        for plus in range(1, num):
            ...
            training_input_color_image_1.append(sv_images[n])
            training_input_color_image_2.append(sv_images[n + plus])
