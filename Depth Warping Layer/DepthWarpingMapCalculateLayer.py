...
class DepthWarpingMapCalculateLayer(Layer):
    def __init__(self, intrinsic_matrix, **kwargs):
        super(DepthWarpingMapCalculateLayer, self).__init__(**kwargs)
    ...
    def call(self, x):
        ...
        warped_depth_map = _depth_warping(intrinsic_matrix, depth_map_1, depth_map_2, translation_vector,
                                          rotation_matrix)
...
def _depth_warping(intrinsic_matrix, depth_map_1, depth_map_2, translation_vectors, rotation_matrices):
    ...
    depth_map_2_calculate, updates = theano.scan(
        fn=lambda W, M, u, v, z_1,: W[2, 0] + z_1 * (M[2, 0] * u + M[2, 1] * v + M[2, 2]),
        outputs_info=None,
    ...
    #
    # u_2, updates = theano.scan(fn=lambda W, M, u, v, z_1, z_2_calculate,: (z_1 * (M[0, 0] * u + M[0, 1] * v + M[0, 2]) +
    #                                                                        W[0, 0]) / z_2_calculate,
    #                            outputs_info=None,
    #                            sequences=[W, M, x_grid, y_grid, depth_map_1,
    #                                       depth_map_2_calculate])  # masked_depth_map_2_calculate
    #
    # v_2, updates = theano.scan(fn=lambda W, M, u, v, z_1, z_2_calculate,: (z_1 * (M[1, 0] * u + M[1, 1] * v + M[1, 2]) +
    #                                                                        W[1, 0]) / z_2_calculate,
    #                            outputs_info=None,
    #                            sequences=[W, M, x_grid, y_grid, depth_map_1,
    #                                       depth_map_2_calculate])  # masked_depth_map_2_calculate
    #
    # depth_map_1_calculate, updates = theano.scan(
    #     fn=lambda W_2, M_2, u, v, z_2,: W_2[2, 0] + z_2 * (M_2[2, 0] * u + M_2[2, 1] * v + M_2[2, 2]),
    #     outputs_info=None,
    ...
    #
    # depth_map_1_transformed_flat = _interpolate(depth_map_1_calculate, u_2_flat,
    #                                             v_2_flat)  # masked_depth_map_1_calculate

    return depth_map_2_calculate
